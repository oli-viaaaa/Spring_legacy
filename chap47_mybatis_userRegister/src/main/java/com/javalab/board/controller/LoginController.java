...
public class LoginController {
	 ...
	 */
	@GetMapping("/login.do")
	public String loginForm(UserVo userVo, Model model, HttpSession session){
		log.info("loginForm 메소드 userVo.toString() " + userVo.toString());
	...
	public String login(UserVo vo, 
						HttpSession session,			// 사용자 정보 세션에 기록
						HttpServletResponse response,	// 쿠키 때문 저장용 
						Model model){
		...
		UserVo userVo = service.getUserByIdPwd(vo);
		...
		if(userVo != null) {
			...
			if(previousUrl != null) {
				...
				returnUrl = "redirect:" + "/login/welcome.do"; // 임시로 환영 페이지로 이동
			}
	...
	public String welcome(UserVo vo, Model model) {
